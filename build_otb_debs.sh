...
cmake "../${OTB_PKG}/SuperBuild" \
  ...
  -DOTB_WRAP_PYTHON=ON \
  -DBUILD_TESTING=OFF \
  -DOTB_CONFIGURE_OPTIONS="-DUSE_SYSTEM_GDAL=ON -DUSE_SYSTEM_QT=ON -DUSE_SYSTEM_BOOST=ON -DUSE_SYSTEM_TIFF=ON -DUSE_SYSTEM_PNG=ON -DUSE_SYSTEM_JPEG=ON -DUSE_SYSTEM_ZLIB=ON -DUSE_SYSTEM_EXPAT=ON -DUSE_SYSTEM_CURL=ON -DUSE_SYSTEM_OPENTHREADS=ON -DUSE_SYSTEM_PYTHON=ON -DUSE_SYSTEM_ITK=OFF -DUSE_SYSTEM_FREETYPE=ON -DUSE_SYSTEM_OPENJPEG=ON -DUSE_SYSTEM_GEOS=ON -DUSE_SYSTEM_XTIFF=ON -DOTB_BUILD_DEFAULT_MODULES=ON -DModule_OTBImageManipulation=ON -DModule_OTBImageStatistics=ON -DModule_OTBMathParser=ON -DModule_OTBProjection=ON -DModule_OTBTransform=ON"
